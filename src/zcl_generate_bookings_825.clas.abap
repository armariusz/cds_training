CLASS zcl_generate_bookings_825 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_bookings_825 IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_825.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_825.
*   insert the new table entries
    INSERT ztbooking_825 FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_825 INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
